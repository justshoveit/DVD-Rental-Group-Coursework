...
namespace SAMS.Controllers
{
    ...
    public class AttendanceReportController : Controller
    {
        ...
        {
            var list = await _repo.GetModules();
            List<SelectListItem> ModuleDDL = list.Select(x => new SelectListItem
            {
                Text = x.Name,
                Value = x.Id.ToString()
            }).ToList();
            ModuleDDL.Insert(0, new SelectListItem { Text = "Select Module", Value = "" });
            ViewBag.ModuleDDL = ModuleDDL;
        ...
        public async Task<ActionResult> Details(StudentReport data)
        {
            ...
            List<AttendanceReport> report = await _reportRepo.AttendanceReport(data);
        ...
        public async Task<JsonResult> GetStudentList(int moduleId)
        {
            try
            {
                List<SAMS.Models.Student> students;
                students = await _repo.GetStudents(moduleId);
                var list = students.Select(x => new SelectListItem { Text = x.Name, Value = x.Id.ToString() }).ToList();
